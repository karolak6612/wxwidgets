...
# Content of the C++ files (re-using from previous step as they are the same)
cpp_files_content = {
...
analyzed_files_data = []
file_descriptions = {
    "wxwidgets/palette_creature.cpp": "(Already analyzed for UI-06) Implements `CreaturePalettePanel` which includes controls for spawn time and radius when the \"Spawn Brush\" mode is active, used for creating new spawns.",
    "wxwidgets/palette_creature.h": "(Already analyzed for UI-06) Header for `CreaturePalettePanel`.",
    "wxwidgets/old_properties_window.cpp": "(Already analyzed for UI-04) Contains a constructor variant `OldPropertiesWindow(Spawn* spawn)` that allows editing only the spawn radius.",
    "wxwidgets/old_properties_window.h": "(Already analyzed for UI-04) Header for `OldPropertiesWindow`."
}
...
- "Group Management" in this context means managing the list of creatures that belong to a single spawn point.""",
    ...
    "boilerplate_coder_ai_prompt": """Your task is to implement the Qt6 UI components for creating and editing creature spawns. This involves utilizing controls on the Creature Palette for new spawns and creating a dedicated dialog for editing existing spawns.
...
output_filename = "enhanced_wbs_yaml_files/UI-08.yaml"
