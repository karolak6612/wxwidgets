...
# Provided WBS data as a multi-line string
wbs_data_string = """work_breakdown_structure:
...
# Corrected WBS entry for LOGIC-04 (Waypoint System)
wbs_logic_04_waypoint = """
  - section: "Editor Behavior"
    id: "LOGIC-04"
    title: "Port Waypoint System"
    input_files: "waypoints.h/cpp, editor.cpp (for waypoint brush logic), map_display.cpp (for rendering)"
    dependencies: "UI-EVENT-01, CORE-01"
    definition_of_done: |
      The `Waypoint` and `Waypoints` classes are ported to `mapcore`. The `WaypointBrush` is functional. Waypoints are rendered on the `MapView`.
    boilerplate_coder_ai_prompt: |
      Port the `Waypoint` and `Waypoints` classes to `mapcore`. Implement rendering of waypoints in `MapView::paintGL`. Port the `WaypointBrush` logic, ensuring it creates an `Action` when waypoints are added or removed.
"""

# A custom representer for multiline strings
...
def parse_list_string(s, remove_backticks_from=None): # remove_backticks_from is not used here, kept for signature consistency if needed later
    if not s or s == "None":
        return None if s == "None" else []

    # Always remove backticks from all items in the list for input_files
    items = [item.strip().replace('`','') for item in s.split(',')]
...
def main():
    output_dir = "current_wbs_yaml_output"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    main_data_yaml = None
    try:
        main_data_yaml = yaml.safe_load(wbs_data_string)
        if not isinstance(main_data_yaml, dict) or 'work_breakdown_structure' not in main_data_yaml:
            print(f"Error: Main WBS data is not in the expected dictionary format. Parsed type: {type(main_data_yaml)}")
            return
        tasks = main_data_yaml.get('work_breakdown_structure', [])
        if not isinstance(tasks, list):
            print(f"Error: 'work_breakdown_structure' is not a list. Found type: {type(tasks)}")
            return
    except yaml.YAMLError as e:
        print(f"Error parsing main WBS string: {e}")
        return
    except Exception as e:
        print(f"An unexpected error occurred during main WBS parsing: {e}")
        return

    try:
        logic_04_task_list = yaml.safe_load(wbs_logic_04_waypoint)
        if logic_04_task_list and isinstance(logic_04_task_list, list) and len(logic_04_task_list) > 0:
            logic_04_task_data = logic_04_task_list[0]
            inserted = False
            for i, task_iter in enumerate(tasks):
                if task_iter.get('id') == 'LOGIC-03':
                    tasks.insert(i + 1, logic_04_task_data)
                    inserted = True
                    break
            if not inserted:
                tasks.append(logic_04_task_data)
        else:
            print("Error: LOGIC-04 waypoint data is not in the expected list format or is empty.")
    except yaml.YAMLError as e:
        print(f"Error parsing LOGIC-04 WBS string: {e}")

    for task_data in tasks:
        task_id = task_data.get('id')
        if not task_id:
            print(f"Skipping task due to missing ID: {task_data.get('title')}")
            continue

        input_files_str = task_data.get('input_files', '')
        # parse_list_string will remove backticks from items in input_files list
        input_files_list = parse_list_string(input_files_str)

        dependencies_str = task_data.get('dependencies', 'None')
        if dependencies_str == "ALL":
            dependencies_list = "ALL"
        else:
            # For dependencies, backticks are not mentioned as needing removal.
            # So, we use a version of parse_list_string that doesn't remove them,
            # or rather, the existing parse_list_string is fine as it only removes backticks for input_files.
            # Let's simplify: parse_list_string currently removes backticks from ALL lists.
            # This should be fine for dependencies too, as they are usually IDs.
            dependencies_list = parse_list_string(dependencies_str)
        ...
        yaml_content = {
            ...
            'input_files': input_files_list if input_files_list is not None else [],
            'dependencies': dependencies_list,
            'definition_of_done': task_data.get('definition_of_done', '').strip(),
            'boilerplate_coder_ai_prompt': task_data.get('boilerplate_coder_ai_prompt', '').strip(),
        }

        filepath = os.path.join(output_dir, f"{task_id}.yaml")
        try:
            with open(filepath, 'w') as f:
                yaml.dump(yaml_content, f, sort_keys=False, width=1000, allow_unicode=True)
        except Exception as e:
            print(f"Error writing YAML for {task_id}: {e}")

if __name__ == '__main__':
    main()
    print("WBS YAML generation script finished.")
