...
2. Move the source code of all listed input files (`position.h`, `tile.h/cpp`, `item.h/cpp`, etc.) into the `mapcore` library's source tree.
...
""",
    "definition_of_done": """\
A static library named `mapcore` is created and compiles successfully via CMake.
Key requirements:
- Contains all specified data model classes (Position, Tile, Item, Map, Creature, etc.).
...
The primary goal is to remove ALL wxWidgets dependencies.
...
7. Pay attention to `main.h`; it must be stripped of wx-specific content for `mapcore` compilation. Create a `mapcore_utils.h` or similar if needed for shared constants/enums previously in `main.h`.
8. Global item/creature definitions (`g_items`, `g_creatures`) can be declared `extern` for now.
9. The existing custom binary serialization logic must be preserved but use standard types.
10. Ensure the `mapcore` library compiles cleanly as a static library using CMake.\
"""
}
