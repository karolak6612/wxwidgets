...
yaml_data = {
    ...
    "dependencies": [
        "CORE-03", # Defines the OTBM/OTMM saver and loader implementations
        "TEST-01", # Core data structures (Map, Tile, ItemData etc.) should be tested before testing their I/O
        # Potentially depends on all tasks that define data stored in the map:
        "CORE-01",
        "CORE-02",
        "LOGIC-04", # Waypoints
        "LOGIC-05", # Houses
    ...
    "input_files": [], # No direct wxWidgets C++ files are ported; tests are new against OTBM/OTMM specs.
...
This task is for creating new unit tests to validate the Qt6 implementation of OTBM (OpenTibia Binary Map) and OTMM (OpenTibia MiniMap) file saving and loading. These tests will ensure data integrity during serialization and deserialization processes.\
""",
    "definition_of_done": [
...
output_file_path = "enhanced_wbs_yaml_files/TEST-03.yaml" # Corrected from TEST-02 to TEST-03
