...
QIcon RawItemsPaletteTab::getItemIcon(quint16 itemId) const
{
    ...
    if (m_clientDataService && m_clientDataService->getSpriteManager()) {
        ...
        // TODO: Get actual sprite for item and render it
        // QPixmap sprite = spriteManager->getItemSprite(itemId);
    }
...
{
    // Get actual item name from database
    if (m_clientDataService && m_clientDataService->getItemDatabase()) {
        const auto& itemData = m_clientDataService->getItemDatabase()->getItemData(itemId);
        if (!itemData.name.isEmpty()) {
            return itemData.name;
        }
    }
    
    // Fallback to generic name if database not available or item not found
