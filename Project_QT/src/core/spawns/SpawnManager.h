...
public:
    ...
    void addSpawn(const Position& pos, const Spawn& spawn);
    ...
    // Spawn access
    Spawn* getSpawn(const Position& pos);
    const Spawn* getSpawn(const Position& pos) const;
...
private:
    ...
    // Helper methods
    void updateTileSpawn(const Position& pos, const Spawn& spawn);
    void clearTileSpawn(const Position& pos);
};
