...
analyzed_files_data = []
file_descriptions = {
    "wxwidgets/live_server.cpp": "Implements `LiveServer` class using Boost.Asio for network communication, client management, message broadcasting, and creating a `LiveLogTab` for UI feedback.",
    "wxwidgets/live_server.h": "Header for `LiveServer`, defining its interface for binding, closing, accepting clients, and broadcasting data.",
    "wxwidgets/main_menubar.cpp": "(Partially relevant) Contains event handlers like `OnStartLive` and `onServerHost` which create wxDialogs to get server configuration (name, port, password) before starting the LiveServer. (Content likely analyzed in UI-01)."
}
...
yaml_output = {
    ...
    "current_functionality_summary": """The wxWidgets version initiates live server hosting via dialogs launched from the `MainMenuBar`. These dialogs collect server name (cosmetic), port, and password.
...
A `LiveLogTab` is created by the `LiveServer` to display server messages, chat, and a list of connected clients. This tab is likely added to the main application's tabbed interface.""",
    ...
    "boilerplate_coder_ai_prompt": """Your task is to create the Qt6 UI for a Live Server Control Panel. This panel will allow the user to configure, start, and stop a live editing server, as well as monitor connected clients and server activity/chat.
...
output_filename = "enhanced_wbs_yaml_files/UI-09.yaml"
