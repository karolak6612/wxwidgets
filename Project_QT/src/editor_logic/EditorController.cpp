...
#include "core/io/OtbmMapIO.h" // Added for FINAL-01
#include "core/actions/TileChangeCommand.h" // Added for TASK_001
#include "editor_logic/commands/RecordSetGroundCommand.h" // Added for TASK_001
#include "editor_logic/commands/RecordAddRemoveItemCommand.h" // Added for TASK_001
#include "core/actions/BatchCommand.h" // Added for TASK_001
...
void EditorController::initializeDependencies()
{
    ...
    // Get from settings service - AppSettings is available
    ...
    // Get from map when map is loaded
    m_housesManager = nullptr; // Will be set when map is loaded via setMap()
...
void EditorController::recordTileChange(const RME::core::Position& pos,
    ...
    if (!m_map || !m_map->isPositionValid(pos)) {
        qWarning() << "EditorController::recordTileChange: Invalid position" << pos.x << pos.y << pos.z;
        return;
    }
    
    Tile* tile = m_map->getTile(pos);
    if (!tile) {
        qWarning() << "EditorController::recordTileChange: No tile at position" << pos.x << pos.y << pos.z;
        return;
    }
    
    // Create a TileChangeCommand to record the current state
    // The command will capture the current tile state as "old" and apply changes as "new"
    auto command = std::make_unique<RME::core::actions::TileChangeCommand>(
        m_map, pos, tile->deepCopy());
    
    pushCommand(std::move(command));
    
    qDebug() << "EditorController::recordTileChange: Recorded tile change at" << pos.x << pos.y << pos.z;
}
...
void EditorController::recordAddSpawn(const RME::core::spawns::Spawn& spawn) {
    ...
    auto addSpawnCommand = std::make_unique<RME::core::actions::AddSpawnCommand>(
        spawn, m_spawnManager, this);
    
    pushCommand(std::move(addSpawnCommand));
    
    qDebug() << "EditorController::recordAddSpawn: Added spawn at" << spawn.getCenter().toString()
             << "with" << spawn.getCreatureTypes().size() << "creature types";
}
...
void EditorController::recordRemoveSpawn(const RME::core::Position& pos) {
    ...
    const RME::core::spawns::Spawn* existingSpawn = m_spawnManager->getSpawn(pos);
    if (!existingSpawn) {
...
void EditorController::recordUpdateSpawn(const RME::core::Position& pos, const RME::core::spawns::Spawn& oldState, const RME::core::spawns::Spawn& newState) {
    if (!m_map || !m_spawnManager) {
...
void EditorController::recordSetGroundItem(const RME::core::Position& pos, uint16_t newGroundItemId, uint16_t /*oldGroundItemId*/) {
    if (!m_map || !m_map->isPositionValid(pos)) {
        qWarning() << "EditorController::recordSetGroundItem: Invalid position" << pos.x << pos.y << pos.z;
        return;
    }
    
    if (newGroundItemId == 0) {
        qWarning() << "EditorController::recordSetGroundItem: Invalid item ID";
        return;
    }
    
    // Get or create tile at position
    Tile* tile = m_map->getTileForEditing(pos);
    if (!tile) {
        qWarning() << "EditorController::recordSetGroundItem: Failed to get tile for editing at" << pos.x << pos.y << pos.z;
        return;
    }
    
    // Create old ground state (current state)
    std::unique_ptr<RME::core::Item> oldGround = nullptr;
    if (tile->getGround()) {
        oldGround = tile->getGround()->deepCopy();
    }
    
    // Create new ground state
    auto newGround = RME::core::Item::create(newGroundItemId, tile->getItemTypeProvider());
    if (!newGround) {
        qWarning() << "EditorController::recordSetGroundItem: Failed to create item with ID" << newGroundItemId;
        return;
    }
    
    // Create and execute command
    auto command = std::make_unique<RME::core::actions::RecordSetGroundCommand>(
        tile, std::move(newGround), std::move(oldGround), this);
    
    pushCommand(std::move(command));
    
    qDebug() << "EditorController::recordSetGroundItem: Set ground item" << newGroundItemId << "at" << pos.x << pos.y << pos.z;
}
...
void EditorController::recordSetBorderItems(const RME::core::Position& pos,
    ...
    if (!m_map || !m_map->isPositionValid(pos)) {
        qWarning() << "EditorController::recordSetBorderItems: Invalid position" << pos.x << pos.y << pos.z;
        return;
    }
    
    if (newBorderItemIds.isEmpty()) {
        qWarning() << "EditorController::recordSetBorderItems: Empty item list";
        return;
    }
    ...
    Tile* tile = m_map->getTileForEditing(pos);
    if (!tile) {
        qWarning() << "EditorController::recordSetBorderItems: Failed to get tile for editing at" << pos.x << pos.y << pos.z;
        return;
    }
    
    // Create a batch command for multiple border items
    auto batchCommand = std::make_unique<RME::core::actions::BatchCommand>("Set Border Items");
    
    for (uint16_t itemId : newBorderItemIds) {
        if (itemId == 0) {
            qWarning() << "EditorController::recordSetBorderItems: Skipping invalid item ID 0";
            continue;
        }
        
        // Create item
        auto item = RME::core::Item::create(itemId, tile->getItemTypeProvider());
        if (!item) {
            qWarning() << "EditorController::recordSetBorderItems: Failed to create item with ID" << itemId;
            continue;
        }
        
        // Create add command for this border item
        auto addCommand = std::make_unique<RME::core::actions::RecordAddRemoveItemCommand>(
            tile, std::move(item), this);
        
        batchCommand->addCommand(std::move(addCommand));
    }
    
    // Only push if we have commands to execute
    if (batchCommand->childCount() > 0) {
        pushCommand(std::move(batchCommand));
        qDebug() << "EditorController::recordSetBorderItems: Set" << newBorderItemIds.size() << "border items at" << pos.x << pos.y << pos.z;
    } else {
        qWarning() << "EditorController::recordSetBorderItems: No valid border items to set";
    }
}

void EditorController::recordAddItem(const RME::core::Position& pos, uint16_t itemId) {
    if (!m_map || !m_map->isPositionValid(pos)) {
        qWarning() << "EditorController::recordAddItem: Invalid position" << pos.x << pos.y << pos.z;
        return;
    }
    
    if (itemId == 0) {
        qWarning() << "EditorController::recordAddItem: Invalid item ID";
        return;
    }
    ...
    Tile* tile = m_map->getTileForEditing(pos);
    if (!tile) {
        qWarning() << "EditorController::recordAddItem: Failed to get tile for editing at" << pos.x << pos.y << pos.z;
        return;
    }
    ...
    auto item = RME::core::Item::create(itemId, tile->getItemTypeProvider());
    if (!item) {
        qWarning() << "EditorController::recordAddItem: Failed to create item with ID" << itemId;
        return;
    }
    
    // Create and execute add command
    auto command = std::make_unique<RME::core::actions::RecordAddRemoveItemCommand>(
        tile, std::move(item), this);
    
    pushCommand(std::move(command));
    
    qDebug() << "EditorController::recordAddItem: Added item" << itemId << "at" << pos.x << pos.y << pos.z;
}

void EditorController::recordRemoveItem(const RME::core::Position& pos, uint16_t itemId) {
    if (!m_map || !m_map->isPositionValid(pos)) {
        qWarning() << "EditorController::recordRemoveItem: Invalid position" << pos.x << pos.y << pos.z;
        return;
    }
    
    if (itemId == 0) {
        qWarning() << "EditorController::recordRemoveItem: Invalid item ID";
        return;
    }
    ...
    Tile* tile = m_map->getTile(pos);
    if (!tile) {
        qWarning() << "EditorController::recordRemoveItem: No tile at position" << pos.x << pos.y << pos.z;
        return;
    }
    
    // Find the item to remove
    RME::core::Item* itemToRemove = nullptr;
    const auto& items = tile->getItems();
    for (const auto& item : items) {
        if (item && item->getID() == itemId) {
            itemToRemove = item.get();
            break;
        }
    }
    
    if (!itemToRemove) {
        qWarning() << "EditorController::recordRemoveItem: Item with ID" << itemId << "not found at" << pos.x << pos.y << pos.z;
        return;
    }
    
    // Create and execute remove command
    auto command = std::make_unique<RME::core::actions::RecordAddRemoveItemCommand>(
        tile, itemToRemove, this);
    
    pushCommand(std::move(command));
    
    qDebug() << "EditorController::recordRemoveItem: Removed item" << itemId << "from" << pos.x << pos.y << pos.z;
}
...
void EditorController::borderizeMap(bool showProgressDialog) {
    ...
    if (showProgressDialog) {
        // Show progress dialog and connect to command
        // Progress dialog functionality exists in Qt - can be connected
        QProgressDialog* progressDialog = new QProgressDialog("Processing map operation...", "Cancel", 0, 100, nullptr);
        progressDialog->setWindowModality(Qt::WindowModal);
        progressDialog->show();
        // TODO: Connect progress updates from command execution
...
bool EditorController::exportMiniMap(const QString& filename, int floor, bool showDialog) {
    ...
    // Determine the floor to export
    int exportFloor = floor;
    if (exportFloor < 0 || exportFloor > 15) {
        // Use current floor if invalid floor specified
        exportFloor = 7; // Default to ground floor
    }
    
    // Create a QImage for the minimap
    int mapWidth = m_map->getWidth();
    int mapHeight = m_map->getHeight();
    
    // Ensure reasonable dimensions
    if (mapWidth <= 0 || mapHeight <= 0 || mapWidth > 65535 || mapHeight > 65535) {
        qWarning("EditorController::exportMiniMap: Invalid map dimensions: %d x %d",
                 mapWidth, mapHeight);
        return false;
    }
    
    // Create image with map dimensions
    QImage minimap(mapWidth, mapHeight, QImage::Format_ARGB32);
    minimap.fill(Qt::transparent);
    
    // Render the map to the image
    QPainter painter(&minimap);
    
    // Iterate through all tiles on the specified floor
    for (int x = 0; x < mapWidth; ++x) {
        for (int y = 0; y < mapHeight; ++y) {
            RME::core::Position pos(x, y, exportFloor);
            const RME::core::Tile* tile = m_map->getTile(pos);
            
            if (tile) {
                // Determine tile color based on its properties
                QColor tileColor;
                
                if (tile->hasGround()) {
                    // Ground tiles are typically brown/green
                    tileColor = QColor(150, 100, 50); // Brown
                } else if (tile->isBlocking()) {
                    // Blocking tiles are typically gray
                    tileColor = QColor(100, 100, 100); // Gray
                } else if (tile->hasItems()) {
                    // Tiles with items are typically blue
                    tileColor = QColor(50, 100, 150); // Blue
                } else {
                    // Empty tiles are transparent
                    continue;
                }
                
                // Draw the tile as a single pixel
                painter.setPen(tileColor);
    ...
    // Save the image
    bool success = minimap.save(filename);
    
    if (success) {
        qInfo("EditorController::exportMiniMap: Successfully exported minimap to %s",
              qUtf8Printable(filename));
    } else {
        qWarning("EditorController::exportMiniMap: Failed to save minimap to %s",
                qUtf8Printable(filename));
    }
...
bool EditorController::newMap(int width, int height, const QString& name) {
    if (!m_map) {
        qWarning() << "EditorController::newMap: No map instance available";
        return false;
    }
    ...
    qInfo() << "EditorController::newMap: Created new map" << width << "x" << height << "named" << name;
    emit mapLoaded(QString()); // Empty filename for new map
    
    return true;
}

bool EditorController::loadMap(const QString& filename) {
    if (filename.isEmpty()) {
        qWarning() << "EditorController::loadMap: Empty filename provided";
        return false;
    }
    
    if (!m_map || !m_assetManager) {
        qWarning() << "EditorController::loadMap: Map or AssetManager not available";
        return false;
    }
    
    // Get app settings
    RME::core::settings::AppSettings* settings = m_appSettings;
    if (!settings) {
        qWarning() << "EditorController::loadMap: AppSettings not available";
        return false;
    }
    
    // Create OTBM map loader
    RME::core::io::OtbmMapIO mapIO;
    
    // Load the map
    bool success = mapIO.loadMap(filename, *m_map, *m_assetManager, *settings);
    ...
    if (success) {
        ...
        qInfo() << "EditorController::loadMap: Successfully loaded map from" << filename;
        emit mapLoaded(filename);
        
        return true;
    } else {
        qWarning() << "EditorController::loadMap: Failed to load map from" << filename 
                   << "Error:" << mapIO.getLastError();
        return false;
    }
}

bool EditorController::saveMap(const QString& filename) {
    QString saveFilename = filename;
    
    // If no filename provided, use current filename
    if (saveFilename.isEmpty()) {
        saveFilename = m_currentMapFilename;
    }
    
    // If still no filename, this is an error
    if (saveFilename.isEmpty()) {
        qWarning() << "EditorController::saveMap: No filename provided and no current filename";
        return false;
    }
    
    return saveMapAs(saveFilename);
}

bool EditorController::saveMapAs(const QString& filename) {
    if (filename.isEmpty()) {
        qWarning() << "EditorController::saveMapAs: Empty filename provided";
        return false;
    }
    
    if (!m_map || !m_assetManager) {
        qWarning() << "EditorController::saveMapAs: Map or AssetManager not available";
        return false;
    }
    ...
    RME::core::settings::AppSettings* settings = m_appSettings;
    if (!settings) {
        qWarning() << "EditorController::saveMapAs: AppSettings not available";
        return false;
    }
    
    // Create OTBM map saver
    RME::core::io::OtbmMapIO mapIO;
    
    // Save the map
    bool success = mapIO.saveMap(filename, *m_map, *m_assetManager, *settings);
    ...
    if (success) {
        ...
        qInfo() << "EditorController::saveMapAs: Successfully saved map to" << filename;
        emit mapSaved(filename);
        
        return true;
    } else {
        qWarning() << "EditorController::saveMapAs: Failed to save map to" << filename 
                   << "Error:" << mapIO.getLastError();
        return false;
    }
}

bool EditorController::closeMap() {
    if (!m_map) {
        qWarning() << "EditorController::closeMap: No map instance available";
        return false;
    }
    ...
    qInfo() << "EditorController::closeMap: Map closed";
    emit mapClosed();
    
    return true;
}

QString EditorController::getCurrentMapFilename() const {
    return m_currentMapFilename;
}

bool EditorController::isMapModified() const {
    return m_mapModified;
}

void EditorController::setMapModified(bool modified) {
    if (m_mapModified != modified) {
        m_mapModified = modified;
        emit mapModifiedChanged(modified);
    }
}
