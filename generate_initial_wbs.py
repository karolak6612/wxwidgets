...
def parse_list_string(s, remove_backticks_from=None):
    if not s or s == "None":
        return None if s == "None" else []
    items = [item.strip() for item in s.split(',')]
    if remove_backticks_from:
        processed_items = []
        for item in items:
            # Remove backticks globally for items in the list
            item_cleaned = item.replace('CMakeLists.txtFINAL-02', '') handles this.
        input_files_str = task_data.get('input_files', '')
        input_files_list = parse_list_string(input_files_str) # Backticks are handled by the replace in parse_list_string
