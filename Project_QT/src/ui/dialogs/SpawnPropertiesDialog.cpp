...
SpawnPropertiesDialog::SpawnPropertiesDialog(QWidget* parent, RME::core::spawns::SpawnData* spawnDataCopy)
    : QDialog(parent)
    , m_spawnDataCopy(spawnDataCopy)
{
    Q_ASSERT(m_spawnDataCopy);
...
void SpawnPropertiesDialog::loadSpawnData() {
    ...
    // Load spawn radius
    m_spawnRadiusSpin->setValue(m_spawnDataCopy->getRadius());
    
    // Update info label
    QString info = tr("Spawn at position %1")
                      .arg(m_spawnDataCopy->getCenter().toString());
...
void SpawnPropertiesDialog::saveSpawnData() {
    ...
    // Save spawn radius
    m_spawnDataCopy->setRadius(m_spawnRadiusSpin->value());
}
...
void SpawnPropertiesDialog::accept() {
    if (validateInput()) {
        saveSpawnData();
        emit spawnDataModified(m_spawnDataCopy);
        QDialog::accept();
    }
}
...
void SpawnPropertiesDialog::createBackup() {
    if (m_spawnDataCopy) {
        m_originalSpawnData = new RME::core::spawns::SpawnData(m_spawnDataCopy->deepCopy());
    }
}

void SpawnPropertiesDialog::restoreBackup() {
    if (m_originalSpawnData && m_spawnDataCopy) {
        *m_spawnDataCopy = *m_originalSpawnData;
