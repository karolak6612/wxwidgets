...
void ItemPropertiesDialog::onAddContainerItem() {
    ...
    RME::ui::dialogs::ItemFinderDialogQt dialog(this, m_clientDataService); // Pass actual ClientDataService
...
void ItemPropertiesDialog::onResetToDefaults() {
    ...
    if (result == QMessageBox::Yes) {
        m_attributesTable->setRowCount(0);
        // Load default attributes from item data
        if (m_clientDataService && m_clientDataService->getItemDatabase()) {
            const auto& itemData = m_clientDataService->getItemDatabase()->getItemData(m_item->getID());
            // Apply default attributes from itemData if needed
        }
        markAsModified();
    }
}
