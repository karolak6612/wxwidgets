...
void RemoveSpawnCommand::redo() {
    ...
    if (!m_hasBackup) {
        const RME::core::spawns::SpawnData* existingSpawn = m_spawnManager->getSpawn(m_position);
        if (!existingSpawn) {
        ...
        m_backupSpawnData = *existingSpawn;
        m_hasBackup = true;
        
        qDebug() << "RemoveSpawnCommand::redo: Backed up spawn at" << m_position.toString()
                 << "with" << m_backupSpawnData.creatures.size() << "creature types";
    }
    
    // Remove the spawn from the spawn manager
    bool success = m_spawnManager->removeSpawn(m_position);
    if (!success) {
        qWarning("RemoveSpawnCommand::redo: Failed to remove spawn at position (%d, %d, %d).",
                 m_position.x, m_position.y, m_position.z);
        setText(QObject::tr("Remove Spawn (Failed)"));
        return;
    }
...
void RemoveSpawnCommand::undo() {
    ...
    bool success = m_spawnManager->addSpawn(m_backupSpawnData);
    if (!success) {
        qWarning("RemoveSpawnCommand::undo: Failed to restore spawn at position (%d, %d, %d).",
                 m_position.x, m_position.y, m_position.z);
        return;
    }
