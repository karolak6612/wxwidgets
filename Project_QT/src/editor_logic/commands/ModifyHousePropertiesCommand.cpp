...
{
    if (!m_housesManager || m_houseId == 0) {
        qWarning("ModifyHousePropertiesCommand: Initialization with null houses manager or invalid house ID.");
        setErrorText("Modify House Properties");
        return;
    }
...
void ModifyHousePropertiesCommand::redo() {
    if (!validateMembers() || !m_housesManager) {
